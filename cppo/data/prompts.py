...
_CODE_PLAN_AND_SOLVE_STDIN_TEMPLATE = """\
You are an expert Python programmer.

**Problem:**
{problem}

First understand the programming problem and devise a plan. Then carry out the plan by writing a complete Python solution.

Write a complete Python solution that reads from standard input and writes to standard output.
Enclose your code in a ```python``` code block."""

_CODE_PLAN_AND_SOLVE_CALL_TEMPLATE = """\
You are an expert Python programmer.

**Problem:**
{problem}

First understand the programming problem and devise a plan. Then carry out the plan by writing the implementation.

Implement the Python function `{entry_point}`.
Do not use input() or print(); return the answer from the function.
Enclose your code in a ```python``` code block."""

_CODE_OBSERVATION_TEMPLATE = """\
You are an expert competitive-programming strategist.

## Problem
{problem}

Brainstorm {num_observations} useful observations for solving this problem.
Focus on constraints, invariants, representations, edge cases, and algorithmic hooks.
Do not write code.
Do not give the final answer.
Output a numbered list only."""

_CODE_OBSERVATION_NL_SOLUTION_TEMPLATE = """\
You are an expert competitive-programming strategist.
...
Use these observations to write one concise natural-language solution.
Do not write code.
Do not use pseudocode.
Do not reveal any final literal answer."""

_CODE_OBSERVATION_CODE_STDIN_TEMPLATE = """\
You are an expert Python programmer.
...
_CODE_OBSERVATION_CODE_CALL_TEMPLATE = """\
You are an expert Python programmer.
...
def make_plan_and_solve_prompt(
    problem: str,
    domain: str = "code",
    io_mode: str = "stdin",
    entry_point: str | None = None,
) -> str:
    """Build a zero-shot Plan-and-Solve prompt for the base solver.

    This is the Wang et al. Plan-and-Solve style baseline: the model is asked
    inside the solve prompt to devise a plan and then produce the solution,
    without a separate planner model or external plan tuple.
    """
    if domain != "code":
        raise ValueError("Plan-and-Solve prompt is currently implemented for code problems only")
    if io_mode == "call_based":
        if entry_point is None:
            raise ValueError("entry_point is required when io_mode='call_based'")
        return _CODE_PLAN_AND_SOLVE_CALL_TEMPLATE.format(
            problem=problem,
            entry_point=entry_point,
        )
    return _CODE_PLAN_AND_SOLVE_STDIN_TEMPLATE.format(problem=problem)


def _format_observations(observations: list[str] | tuple[str, ...]) -> str:
    if not observations:
        return "- No additional observations."
    return "\n".join(f"- {obs}" for obs in observations)


def make_observation_prompt(problem: str, *, num_observations: int, domain: str = "code") -> str:
    """Build the observation-generation prompt used by the PlanSearch-style baseline."""
    if domain != "code":
        raise ValueError("Observation PlanSearch prompt is currently implemented for code problems only")
    return _CODE_OBSERVATION_TEMPLATE.format(
        problem=problem,
        num_observations=num_observations,
    )


def make_observation_nl_solution_prompt(
    problem: str,
    observations: list[str] | tuple[str, ...],
    *,
    domain: str = "code",
) -> str:
    """Build a prompt that turns an observation combo into a natural-language solution."""
    if domain != "code":
        raise ValueError("Observation NL-solution prompt is currently implemented for code problems only")
    return _CODE_OBSERVATION_NL_SOLUTION_TEMPLATE.format(
        problem=problem,
        observations=_format_observations(observations),
    )


def make_observation_code_prompt(
    problem: str,
    observations: list[str] | tuple[str, ...],
    nl_solution: str,
    *,
    ...
    """Build a code-generation prompt from observations plus a natural-language solution."""
    if domain != "code":
        raise ValueError("Observation code prompt is currently implemented for code problems only")
    formatted_observations = _format_observations(observations)
    if io_mode == "call_based":
        if entry_point is None:
            raise ValueError("entry_point is required when io_mode='call_based'")
        return _CODE_OBSERVATION_CODE_CALL_TEMPLATE.format(
            problem=problem,
            observations=formatted_observations,
            nl_solution=nl_solution,
            entry_point=entry_point,
        )
    return _CODE_OBSERVATION_CODE_STDIN_TEMPLATE.format(
        problem=problem,
        observations=formatted_observations,
        nl_solution=nl_solution,
    )
